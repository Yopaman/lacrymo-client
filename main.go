...
func diffieHellman(enc *netstring.Encoder, dec *netstring.Decoder, keyPath string) {
	...
	B, _ := new(big.Int).SetString(jsonResp.B, 10)
...
func main() {
	...
	cBlock, err := aes.NewCipher(cKaes)
	if err != nil {
		panic("crypto error : " + err.Error())
	}
	cStream = cipher.NewCTR(cBlock, cKiv)
	cMac = hmac.New(sha256.New, cKmac)
	sBlock, err := aes.NewCipher(sKaes)
	if err != nil {
		panic("crypto error" + err.Error())
	}
	sStream = cipher.NewCTR(sBlock, sKiv)
	sMac = hmac.New(sha256.New, sKmac)
	...
	diffieHellman(enc, dec, "../private-key.pem")
